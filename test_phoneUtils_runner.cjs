...
let exported;
try {
    exported = vm.runInContext(wrapped, sandbox, { filename: filePath });
} catch (e) {
    console.error('Error evaluating phoneUtils.js in wrapper:', e);
    process.exit(1);
}

if (!exported || typeof exported.formatPhoneNumber !== 'function') {
...
const tests = [
    ...
    { label: 'navidial 0570', num: '0570123456' },
    { label: 'invalid (starts +81)', num: '+811234567890' }
];

for (const t of tests) {
    try {
            const res = exported.formatPhoneNumber(t.num);

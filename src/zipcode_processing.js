...
(($) => {
    ...
    $.zipcode_to_address = (zip_code) => {
        ...
        // （出力値）
        // .zip_code = 3桁数字＋-＋4桁数字の郵便番号を返す（デジタルアドレスは返さない）。
        // .address = 郵便番号に該当する都道府県名＋市町村名＋町名を返す。（入力された郵便番号が個別事業所の場合、デジタルアドレスの場合は番地も含む全住所）
        // .pref = 郵便番号に該当する都道府県名を返す
        // .city = 郵便番号に該当する市町村名を返す
        // .town = 郵便番号に該当する町名を返す
        // .block = デジタルアドレスの場合、番地を返す
        // .other_address = デジタルアドレスの場合、番地よりも先の住所を返す
        // .office = 大口事業所の個別番号の場合、郵便番号に該当する事業所名を返す
        // .zip_code_1 = 郵便番号の1桁目の数字を返す。
        // .zip_code_2 = 郵便番号の2桁目の数字を返す。
        // .zip_code_3 = 郵便番号の3桁目の数字を返す。
        // .zip_code_4 = 郵便番号の4桁目の数字を返す。
        // .zip_code_5 = 郵便番号の5桁目の数字を返す。
        // .zip_code_6 = 郵便番号の6桁目の数字を返す。
        // .zip_code_7 = 郵便番号の7桁目の数字を返す。
        // zip_codeに間違いがある場合は、不要な記号を除去した値を返却する
        // zip_code以外存在しない場合や間違いのある場合はnull値を返却する
    ...
    $.zipcode_separation = (zip_code) => {
        ...
        // （出力値）
        // [0]～[6] = 郵便番号のうち数字部分の1桁目～7桁目を返す。（c桁目と出力される配列の数字が-1になることに注意）
        let separation = [];
        for (let c = 0; c < 8; c++) {

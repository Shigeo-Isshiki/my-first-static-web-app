...
const validateTrackingNumber = (trackingNumber, minLength = 10, maxLength = 14) => {
    ...
    if (!Number.isInteger(minLength) || minLength < 1) throw new Error('最小桁数は1以上の整数である必要があります');
    if (!Number.isInteger(maxLength) || maxLength < minLength) throw new Error('最大桁数は最小桁数以上の整数である必要があります');
    ...
    if (digitsOnly.length < minLength || digitsOnly.length > maxLength) {
        throw new Error(`伝票番号は${minLength}桁以上${maxLength}桁以下の数字である必要があります（現在: ${digitsOnly.length}桁）`);
    }
 ...
 */
const getNextBusinessDay = (baseDate = new Date(), callback) => {
